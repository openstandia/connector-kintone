...
class GroupTest extends AbstractTest {
    ...
    void updateGroupWithNoValues() {
        ...
        // When
        Set<AttributeDelta> affected = connector.updateDelta(GROUP_OBJECT_CLASS, new Uid(currentId, new Name(currentName)), modifications, new OperationOptionsBuilder().build());

        // Then
        assertNull(affected);

        assertEquals(currentId, targetUid.get().getUidValue());
        assertEquals(currentName, targetUid.get().getNameHintValue());
